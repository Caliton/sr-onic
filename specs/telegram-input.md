...
**Usuario:** Caliton interagindo do smartphone para a máquina desktop local através de uma DM do Bot do Telegram, mandando comandos de voz dirigindo o carro pedindo para o agente agir e exigindo receber a resposta de volta em formato de voz (TTS Thalita) para audição rápida sem contato visual no app.
...
| RF-07 | O sistema deve realizar **health check** de Whisper e `ffmpeg` no startup. | Must | Se Whisper ou ffmpeg não estiverem acessíveis (binário ausente, PATH incorreto), logar warning e desabilitar funcionalidade de voz graciosamente. O bot continua operando para texto e documentos. |
| RF-08 | O sistema deve **limpar o diretório `./tmp/`** no startup. | Must | Ao inicializar, todos arquivos residuais de sessões anteriores (PDFs, áudios, MDs) são deletados. Previne acumulo após crashes ou reinicializações abruptas. |
| RF-09 | O sistema deve **ignorar eventos `message:edit`** (mensagens editadas). | Should | Mensagens editadas não reprocessam o pipeline. Um log informativo é registrado para debug, mas nenhuma ação é tomada. A resposta original permanece válida. |
...
| EC-08: Whisper ou ffmpeg não instalado | Usuário envia Voice Note mas Whisper não está disponível na máquina | O health check no startup já desabilitou a funcionalidade de voz. O bot responde: "⚠️ Funcionalidade de voz desabilitada: Whisper não encontrado. Envie sua mensagem como texto." sem crashar. |
| EC-09: Mensagem editada durante processamento | Usuário edita uma mensagem que já está sendo processada pelo pipeline | O sistema ignora eventos `message:edit`. A resposta é baseada na mensagem original. Log informativo registra a edição para auditoria. |
...
- Como acionar o Whisper local a partir do backend Node.js (se SrOnic ainda for Node). *Decisão pendente: usar bridge FFI, ou Child Process nativo na máquina.*
