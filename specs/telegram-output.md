...
**Usuario:** Caliton, via aplicativo de Telegram que se beneficia de uma Timeline dividida limpa pra leitura dos relatórios com suporte a baixar em .MD suas specs, PRDs etc.
...
| RF-05 | O TextOutputStrategy deve tentar enviar com `parse_mode: MarkdownV2` após **sanitização automática** (escape de caracteres especiais: `_`, `*`, `[`, `]`, `(`, `)`, `~`, `` ` ``, `>`, `#`, `+`, `-`, `=`, `|`, `{`, `}`, `.`, `!`). Se o `ctx.reply` falhar (erro 400), fazer **fallback automático** para texto puro sem formatação. | Should | Mensagens com negrito, código e links funcionam quando possível. Quando o escape falha, o usuário recebe texto legível sem formatação em vez de erro silencioso. |
| RF-06 | Durante operações longas de output (envio de múltiplos chunks, geração de áudio TTS), o sistema deve reenviar `sendChatAction('typing')` ou `sendChatAction('upload_voice')` a cada 4 segundos via `setInterval`. | Should | O indicador de digitação/upload no Telegram nunca expira durante o processamento (a action do TG expira após ~5s sem refresh). O intervalo é limpo após o envio completo. |
...
| EC-04: MarkdownV2 falha no escape | Texto contém combinação de caracteres especiais que o sanitizer não consegue escapar corretamente (ex: URLs com parênteses aninhados) | O `ctx.reply` com `parse_mode: MarkdownV2` retorna erro 400. O sistema captura o erro, loga warning com o trecho problemático, e reenvia a mensagem como texto puro sem `parse_mode`. O usuário recebe o conteúdo legível mesmo sem formatação. |
