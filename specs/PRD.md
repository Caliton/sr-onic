...
O Onic é um agente pessoal de Inteligência Artificial para operar 100% localmente no desktop do usuário. Ele recebe comandos exclusivamente pelo Telegram, processa-os através de um pipeline que suporta múltiplos LLMs dinamicamente, e tem acesso persistente à memória em SQLite.
...
**Usuário primário:** Caliton (proprietário), acessando via dispositivo móvel ou desktop via cliente Telegram, utilizando IDs em whitelist restrita.
...
O usuário envia um chat no Telegram, o SrOnic roda local em background num terminal, chama LLMs, lê Skills em pastas locais, aciona ferramentas e responde no mesmo chat de forma orgânica.
...
| RF-04 | O sistema deve suportar comandos de gerenciamento via chat (`/reset`, `/provider`, `/skills`) | Should | `/reset` limpa contexto da conversa ativa; `/provider <nome>` troca o LLM em runtime; `/skills` lista skills disponíveis. |
| RF-05 | O sistema deve limpar automaticamente o diretório `./tmp/` ao inicializar | Must | No startup, arquivos residuais de sessões anteriores (PDFs, áudios) são deletados. Nenhum arquivo órfão persiste entre reinicializações. |
| RF-06 | O sistema deve realizar health check de dependências críticas (Whisper, ffmpeg) no startup | Should | Se Whisper/ffmpeg não estiverem disponíveis, logar warning e desabilitar funcionalidade de voz sem crashar o processo. |
...
| RNF-03 | Logging estruturado em arquivo | Rotação diária, max 50MB | Logs em `./data/logs/` com formato `[timestamp][level][module] message` para rastreamento pós-execução. |
| RNF-04 | Métricas básicas de operação | Por sessão | Rastrear: mensagens processadas, tokens consumidos (estimado), tool calls executadas, erros por provider. Persistir em tabela `metrics` no SQLite. |
...
| EC-05: Mensagens simultâneas concorrentes | Usuário envia duas mensagens antes da primeira terminar de processar | Mensagens são enfileiradas (FIFO) e processadas sequencialmente. A segunda aguarda a primeira encerrar o Agent Loop antes de iniciar. |
| EC-06: Mensagem editada durante processamento | Usuário edita uma mensagem que já está no pipeline | O sistema ignora `message:edit` por padrão. A resposta é baseada na mensagem original. Um log informativo é registrado. |
