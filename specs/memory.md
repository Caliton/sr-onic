...
O módulo de persistência de estado do SrOnic gerencia tanto as conversas de longo prazo em banco de dados SQLite (`better-sqlite3`) quanto atua como manager da janela de contexto para impedir que o limite maximo do envelope de tokens da IA (Context Window) estoure.
...
| RF-03 | A classe abstrata repassará ao Agent Loop somente mensagens recentes respeitando o limite de `MAX_CONTEXT_TOKENS` (estimativa de tokens). | Must | A janela de contexto é calculada por estimativa de tokens (ex: `chars / 4`) em vez de contagem fixa de mensagens. Isso evita que um PDF transcrito de 10k tokens ocupe todo o contexto mesmo sendo "1 mensagem". Fallback para `MEMORY_WINDOW_SIZE` (contagem) se a estimativa falhar. |
| RF-04 | O banco deve manter uma tabela `schema_version` com versionamento incremental. Migrações são aplicadas automaticamente no startup via `ALTER TABLE` progressivo. | Should | Ao adicionar campos novos (ex: `token_count`, `provider_used`) o banco existente é atualizado sem perda de dados e sem necessidade de deletar o arquivo `.db`. |
...
**Campos adicionais planejados para `messages`:**
- `token_count` (INTEGER, nullable): Estimativa de tokens da mensagem para cálculo de janela de contexto.
- `provider_used` (TEXT, nullable): Provider LLM que gerou a resposta (ex: `gemini`, `deepseek`). Preenchido apenas para `role='assistant'`.

**Tabela `schema_version`:**
```sql
schema_version {
  version: INTEGER  -- Número incremental da versão atual do schema
  applied_at: TEXT  -- Timestamp ISO da aplicação
}
```
No startup, o sistema lê a `version` atual e aplica migrações pendentes sequencialmente até a versão mais recente.
...
| EC-04: Migração de schema em banco existente | Banco com schema v1 é aberto após atualização do código que espera v2 | O sistema detecta `schema_version < CURRENT_VERSION`, aplica `ALTER TABLE` sequenciais e atualiza a versão. Se a migração falhar, loga erro crítico e aborta o startup sem corromper o banco. |

---

## 12. Segurança e Privacidade

- **Arquivos DB Sensíveis:** O `SrOnic.db` jamais pode ir pro Git (Adicionar no `.gitignore` /data).
