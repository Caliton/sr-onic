...
**Usuario Primario:** Caliton, através da pasta Filesystem para inserir diretórios customizados com `.md`, e o bot interno (Loader e Executor) para lidar nos backgrounds da arquitetura.
...
| RF-04 | O sistema deve aplicar um **fast-path** de detecção de skill antes de consultar o LLM no Router. Comandos explícitos (ex: `/prd`, `/git`, `/code`) e keywords mapeadas devem resolver diretamente para a skill correspondente sem chamada LLM. | Should | Mensagem `/prd Crie uma spec` resolve imediatamente para `prd-manager` sem custo de API. Mapa de atalhos é configurável no frontmatter da skill (`triggers: ["/prd", "crie uma spec"]`). |
| RF-05 | O `SkillRouter` deve manter um **cache em memória** de decisões de routeamento dentro da mesma sessão. | Could | Se o usuário enviou 3 mensagens consecutivas sobre código e o Router já resolveu `code-analyzer`, as próximas mensagens similares reutilizam a decisão sem nova chamada LLM. O cache expira ao trocar de conversa ou após timeout configurável. |
...
| EC-04: Custo desnecessário no Router para mensagens triviais | Usuário envia "oi", "obrigado", "valeu" ou emojis soltos | O fast-path detecta ausência de keywords de skill e retorna `null` diretamente, sem invocar o LLM Router. O agente responde como chatbot genérico. |
