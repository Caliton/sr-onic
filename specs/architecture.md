...
```mermaid
graph TB
    User([👤 Usuário Caliton])
    Telegram[📱 Telegram Client]
    System[🏗️ SrOnic Engine]
...
graph TB
    ...
    ConvR & MsgR --> DB
    Loop --> OutputH
    Logger -.->|Transversal| Controller & Loop & MemM
```
...
5.  **Strategy:** No `TelegramOutputHandler` para decidir entre enviar texto puro, chunks, arquivos ou áudio.
6.  **Registry:** No sistema de Skills e Tools para registro dinâmico de capacidades.
7.  **Queue (FIFO):** No `MessageQueue` para enfileirar mensagens recebidas durante processamento ativo, garantindo execução sequencial sem concorrência de contexto.
8.  **Circuit Breaker:** No `ProviderFactory` para fallback inteligente entre LLMs — após N falhas consecutivas de um provider, desabilitá-lo temporariamente e redirecionar para o próximo.
...
| Falha na API de LLM | Alto | Implementação de `fallback` + Circuit Breaker no `ProviderFactory`. |
| Vazamento de Memória (Node) | Médio | Gerenciamento estrito de Buffer de áudio e exclusão de arquivos TMP. |
| Estouro de Contexto IA | Médio | Truncamento baseado em estimativa de tokens no `MemoryManager` via `MAX_CONTEXT_TOKENS`. |
| Mensagens concorrentes corrompendo contexto | Alto | Fila FIFO (`MessageQueue`) entre Input e Controller, processando uma mensagem por vez. |
| Arquivos temporários órfãos após crash | Baixo | Limpeza automática do diretório `./tmp/` no startup antes de iniciar o polling. |
| Dependências de runtime ausentes | Médio | `HealthChecker` no startup valida Whisper, ffmpeg e conectividade com providers. Desabilita graciosamente features indisponíveis. |
